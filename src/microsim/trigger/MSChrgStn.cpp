...
MSChrgStn::MSChrgStn( const std::string& new_ChrgStnID,    const std::vector<std::string>& new_lines, MSLane& new_Lane, SUMOReal new_StartPos, SUMOReal new_EndPos, SUMOReal new_ChrgPower, SUMOReal new_Efficency, SUMOReal new_ChargeInTransit, SUMOReal new_ChargeDelay)
: MSBusStop(new_ChrgStnID, new_lines, new_Lane, new_StartPos, new_EndPos), ChrgPower(new_ChrgPower), Efficency(new_Efficency), ChargeInTransit(new_ChargeInTransit), ChargeDelay(new_ChargeDelay)
{
    if (getChrgPower() < 0)
