...
void MSPModel::DEBUG_PRINT(const Obstacles& obs) {
...
MSPModel::WalkingAreaPaths MSPModel::myWalkingAreaPaths;
...
// model parameters
SUMOReal MSPModel::STRIPE_WIDTH; 
SUMOReal MSPModel::DAWDLING; 
const SUMOReal MSPModel::SAFETY_GAP(1.0);
const SUMOReal MSPModel::LOOKAHEAD_SAMEDIR(4.0); // seconds
const SUMOReal MSPModel::LOOKAHEAD_ONCOMING(10.0); // seconds
const SUMOReal MSPModel::LATERAL_PENALTY(-1);
const SUMOReal MSPModel::SQUEEZE(0.7);
const SUMOReal MSPModel::BLOCKER_LOOKAHEAD(10.0);
const SUMOReal MSPModel::ONCOMIN_PENALTY(-0.5);
const SUMOReal MSPModel::ONCOMIN_PENALTY_FACTOR(0.6);
const SUMOReal MSPModel::ONCOMIN_PATIENCE(30);
const SUMOReal MSPModel::RESERVE_FOR_ONCOMING_FACTOR(0.0);
const SUMOReal MSPModel::MAX_WAIT_TOLERANCE(120);
const SUMOReal MSPModel::LATERAL_SPEED_FACTOR(0.4);
...
MSPModel::add(MSPerson* person, MSPerson::MSPersonStage_Walking* stage, MSNet* net) {
    ...
    if (net != 0 && !active) {
        ...
        DAWDLING = OptionsCont::getOptions().getFloat("pedestrians.dawdling");
...
void 
MSPModel::initWalkingAreaPaths(const MSNet* net) {
...
MSPModel::NextLaneInfo 
MSPModel::getNextLane(const Pedestrian& ped, const MSLane* currentLane, const MSLane* prevLane) {
...
MSPModel::Obstacles
MSPModel::mergeObstacles(const Obstacles& obs1, const Obstacles& obs2, int dir) {
...
MSPModel::Obstacles
MSPModel::getNeighboringObstacles(const Pedestrians& pedestrians, int egoIndex, int stripes) {
...
const MSPModel::Obstacles&
MSPModel::getNextLaneObstacles(NextLanesObstacles& nextLanesObs, const MSLane* nextLane, int stripes, int nextDir, 
...
void 
MSPModel::moveInDirection(SUMOTime currentTime, std::set<MSPerson*>& changedLane, int dir) {
    for (ActiveLanes::iterator it_lane = myActiveLanes.begin(); it_lane != myActiveLanes.end(); ++it_lane) {
        ...
        for (int ii = 0; ii < pedestrians.size(); ++ii) {
            ...
            if (link != 0 
                ...
                Obstacles closedLink(stripes, Obstacle(dir == FORWARD ? lane->getLength() - POSITION_EPS : POSITION_EPS, 0, "closedLink"));
...
// ===========================================================================
// MSPModel::Obstacle method definitions
// ===========================================================================
MSPModel::Obstacle::Obstacle(int dir) :
...
MSPModel::Obstacle::Obstacle(const Pedestrian& ped, int dir) :
...
{ 
    ...
    if (nextRouteEdge != 0) {
        ...
        myDir = (myX <= myStage->getCurrentEndPos()) ? FORWARD : BACKWARD;
    }
    if (myDir == FORWARD) {
...
bool
MSPModel::Pedestrian::moveToNextLane(SUMOTime currentTime) {
...
void 
MSPModel::Pedestrian::walk(const Obstacles& obs, SUMOTime currentTime) {
...
void 
MSPModel::Pedestrian::updateLocation() {
    const SUMOReal shift = myY + (STRIPE_WIDTH - myLane->getWidth()) * 0.5;
    // XXX rewrite to let MSPerson query MSPModel for a position instead
    myStage->updateLocationSecure(myPerson, myLane, myX, shift, myDir, 
            myWalkingAreaPath == 0 ? PositionVector() : myWalkingAreaPath->shape);
}
...
MSPModel::MovePedestrians::execute(SUMOTime currentTime) {
    ...
    MSPModel::moveInDirection(currentTime, changedLane, FORWARD);
    MSPModel::moveInDirection(currentTime, changedLane, BACKWARD);
    // DEBUG
    if (LOG_ALL) {
        for (ActiveLanes::iterator it_lane = myActiveLanes.begin(); it_lane != myActiveLanes.end(); ++it_lane) {
            const MSLane* lane = it_lane->first;
